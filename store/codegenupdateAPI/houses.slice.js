...
export const api_v1_house_read = createAsyncThunk(
  "houses/api_v1_house_read",
  async payload => {
    const response = await apiService.api_v1_house_read(payload)
    return response.data
  }
)
export const api_v1_house_delete = createAsyncThunk(
  "houses/api_v1_house_delete",
  async payload => {
    const response = await apiService.api_v1_house_delete(payload)
    return response.data
  }
)
...
const housesSlice = createSlice({
  ...
  extraReducers: {
    ...
    [api_v1_house_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [api_v1_house_read.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [
    ...
    [api_v1_house_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    },
    [api_v1_house_delete.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [api_v1_house_delete.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
    ...
    [api_v1_house_delete.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
